...
async function tryCreateToken(page, nonce, postId) {
    return page.evaluate(async ({ pvtUrl, n, pid, exp }) => {
        ...
        pvtUrl: `${WP}/wp-json/yui-preview-token/v1/token`,
        n: nonce,
        pid: postId,
...
test.describe('Token creation permissions by WordPress role', () => {
    ...
    test.describe('minimum role: administrator', () => {
        ...
        test.beforeAll(async ({ browser }) => {
            ...
            await page.selectOption('select[name="yuipt_min_capability"]', 'administrator');
        ...
        test.afterAll(async ({ browser }) => {
            ...
            const { page, ctx } = await loginAndGetNonce(browser, 'admin');
            await page.goto(`${WP}/wp-admin/options-general.php?page=yui-preview-token`);
            await page.selectOption('select[name="yuipt_min_capability"]', 'contributor');
            await page.getByRole('button', { name: /save changes/i }).click();
            await page.waitForURL(`${WP}/wp-admin/options-general.php*`);
            await ctx.close();
        });
